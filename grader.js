...
References:
	...
	+ commander.js
		...
		- http://tjholowaychuk.com/post/9103188408/commander-js-nodejs-command-lin
e-interfaces-made-easy
...
var assertURLExists = function(infile) {
	var instr = infile.toString(); 
	console.log("URL name is %s ", instr);
// 	if(!fs.existsSync(instr)) {
// 		console.log("%s does not exist. Exiting.", instr);
// 		process.exit(1); // http://nodejs.org/api/process.html#process_process_exit_code
// 	}
...
var checkHtmlFile = function(htmlfile, checksfile) { 
	...
	for(var ii in checks) {
		var present = $(checks[ii]).length > 0; out[checks[ii]] = present;
	}
	return out; 
};

var checkURLFile = function(urlfile, checksfile) { 
	$ = rest.get(urlfile).on('complete');
	var checks = loadChecks(checksfile).sort();
	var out = {};
	for(var ii in checks) {
		var present = $(checks[ii]).length > 0; out[checks[ii]] = present;
...
// 		.option('-u, --url ', 'URL', assertURLExists, URLFILE_DEFAULT)
		.parse(process.argv);
